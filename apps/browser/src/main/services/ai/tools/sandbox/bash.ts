...
export const bashTool = tool({
  ...
  description: [
    ...
    "The VM is isolated and **cannot install new packages** (no `apk add`, no `pip install`) — only the pre-installed tools below are available.",
    "Available tools:",
    "shell/core (bash, coreutils, findutils, grep, sed, gawk),",
    "network (curl, wget, ca-certificates, iproute2),",
    "vcs (git),",
    "data (jq, sqlite, file),",
    "archive (tar, gzip, xz, zip, unzip — note: .xls is OLE2/CFB, NOT a zip; .xlsx IS a zip),",
